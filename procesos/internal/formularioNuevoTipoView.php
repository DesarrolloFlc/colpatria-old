...
for($i = $an; $i <= 12; $i++){
	...
	if($i > 9)
		$val_m = $i;
?>
						<option value="<?=$val_m?>"><?=$val_m?></option>
<?php
}
?>
					</select>
...
if(isset($funcionarios) && !empty($funcionarios) && is_array($funcionarios)){
	foreach($funcionarios as $funcionario){
		$sel = '';
		if(isset($radInfo['oficial_nombre']) && !is_null($radInfo['oficial_nombre']) && $funcionario['name'] == $radInfo['oficial_nombre'])
			$sel = ' selected="selected"';
?>
						<option value="<?=strtoupper($funcionario['name'])?>"<?=$sel?>><?=strtoupper($funcionario['name'])?></option>
<?php
	}
}
?>
...
?>
					...
					<select id="f_exp_d" name="f_exp_d" title="Fecha de expedici&oacute;n: dia" style="font-size: 12px" onblur="$(this).verificarFechaDoble(event, 'exp', '1');" title="Dia de fecha expedicion">
						<option value="">Dia</option>
					</select>
				</td>
			</tr>
...
?>
			...
			<tr>
				...
				<td>
					<input type="text" id="direccionempresa" name="direccionempresa" style="width: 180px; margin-right: 10px" onkeypress="return validar_letra(event)" title="Direccion oficina" onblur="$(this).ocultarEsteCampo(event);">
					Telefono oficina:
...
?>
			...
			<tr>
				...
				<td>
					<input type="text" id="celularoficinappal" name="celularoficinappal" style="width: 160px" onchange="$(this).checkTamanoTele(event, 10);" maxlength="10" onkeypress="return validar_num(event)" title="Celular oficina"><!--agregar campo llamado celularoficinappal-->
				</td>
			</tr>
			<tr>
...
?>
			...
			<tr>
				...
				<td>
					Inicio
					<select id="f_ini_a" name="f_ini_a" onchange="$(this).verificarFecha(event, 'ini', '0');" style="font-size: 12px" disabled title="Año de inicio"><!--agregar campo llamado cargo_politica_ini-->
						<option value="">Año</option>
<?php
$an = 1900;
$anl = date('Y');
...
?>
					...
					<select id="f_ini_m" name="f_ini_m" onchange="$(this).verificarFecha(event, 'ini', '0');" style="font-size: 12px" disabled title="Mes de inicio">
						<option value="">Mes</option>
<?php
$an = 1;
for($i=$an;$i<=12;$i++){
...
?>
					...
					<select id="f_ini_d" name="f_ini_d" title="Fecha de expedici&oacute;n: dia" style="font-size: 12px" disabled title="Dia de inicio">
						<option value="">Dia</option>
					</select>
					Fin
					<select id="f_fin_a" name="f_fin_a" onchange="$(this).verificarFecha(event, 'fin', '0');" style="font-size: 12px" disabled title="Año de fin"><!--agregar campo llamado cargo_politica_fin-->
						<option value="">Año</option>
<?php
$an = 1900;
...
?>
					...
					<select id="f_fin_m" name="f_fin_m" onchange="$(this).verificarFecha(event, 'fin', '0');" style="font-size: 12px" disabled title="Mes de fin">
						<option value="">Mes</option>
<?php
$an = 1;
for($i=$an;$i<=12;$i++){
...
?>
					...
					<select id="f_fin_d" name="f_fin_d" title="Fecha de expedici&oacute;n: dia" style="font-size: 12px" disabled title="Dia de fin">
						<option value="">Dia</option>
					</select>
				</td>
			</tr>
			<tr>
				<td style="width: 100px;display: table-cell;">Re-escribir direccion oficina:</td>
				<td>
					<input type="text" id="direccionempresa2" name="direccionempresa2" style="width: 180px; margin-right: 10px" onkeypress="return validar_letra(event)" title="Direccion oficina" onblur="$(this).validarCampoReescrito(event, 'input', 'form_fingering', 'direccionempresa', 'Las direcciones de oficina no coinciden por favor validelas.');">
				</td>
			</tr>
			<tr>
			...
			<tr>
				<td style="width: 80px">Re-escribir fecha de expedicion:</td><!--fechasolicitud-->
				<td>
					<select id="f_exp2_a" name="f_exp2_a" onchange="$(this).verificarFecha(event, 'exp2', '1');" style="font-size: 12px" title="text">
						<option value="">Año</option>
<?php
$an = 1900;
$anl = date('Y');
...
?>
					...
					<select id="f_exp2_m" name="f_exp2_m" onchange="$(this).verificarFecha(event, 'exp2', '1');" style="font-size: 12px" title="text">
						<option value="">Mes</option>
<?php
$an = 1;
for($i=$an;$i<=12;$i++){
...
?>
					...
					<select id="f_exp2_d" name="f_exp2_d" onblur="$(this).verificarFechaDoble(event, 'exp', '2');" style="font-size: 12px" title="text">
						<option value="">Dia</option>
					</select>
				</td>
			</tr>
			...
			<tr>
				...
				<td>
					<input type="text" id="telefonoresidencia2" name="telefonoresidencia2" style="width: 130px; margin-right: 10px" onblur="$(this).validarCampoReescrito(event, 'input', 'form_fingering', 'telefonoresidencia', 'Los telefonos no coinciden por favor validelos.');" maxlength="7" onkeypress="return validar_num(event)" title="text">
					Re-escribir Celular:
					<input type="text" id="celular2" name="celular2" style="width: 130px" onblur="$(this).validarCampoReescrito(event, 'input', 'form_fingering', 'celular', 'Los numeros de celular no coinciden, por favor validelos.');" maxlength="10" onkeypress="return validar_num(event)" title="text">
				</td>
			</tr>
			<tr>
...
?>
			...
			<tr>
				...
				<td>
					...
					Monto:
					<input type="text" id="producto_monto[]" name="producto_monto[]" style="width: 80px; margin-right: 5px" onkeypress="return validar_num(event)" disabled title="Monto">
					Ciudad:
	...
	<tr>
		...
		<table>
			...
			<tr>
				...
				<td>
					<input type="text" id="rec_ano[]" name="rec_ano[]" style="width: 50px; margin-right: 5px" onkeypress="return validar_num(event)" disabled title="Año">
					Ramo:
			...
			<tr>
				...
				<td>
					...
					Valor:
					<input type="text" id="rec_valor[]" name="rec_valor[]" style="width: 120px; margin-right: 5px" onkeypress="return validar_num(event)" disabled title="Valor">
				</td>
			</tr>
			<tr>
...
?>
			...
			<tr>
				...
				<td>
					...
					Clave:
					<input type="text" id="clave_inter" name="clave_inter" style="width: 100px" onkeypress="return validar_letra(event)" title="Clave"><!--agregar campo llamado clave_inter-->
				</td>
			</tr>
			<tr>
...
?>
	...
	$('select[name="expuesta_politica"]').change(function(event){
		...
		if($(this).val() == '-1'){
			...
			$('select[name="f_ini_a"]').removeAttr('disabled');
			$('select[name="f_ini_m"]').removeAttr('disabled');
			$('select[name="f_ini_d"]').removeAttr('disabled');
			$('select[name="f_fin_a"]').removeAttr('disabled');
			$('select[name="f_fin_m"]').removeAttr('disabled');
			$('select[name="f_fin_d"]').removeAttr('disabled');
		}else if($(this).val() != ''){
			$('input[name="cargo_politica"]').val('');
			$('input[name="cargo_politica"]').attr('disabled', true);
			$('select[name="f_ini_a"]').attr('disabled', true);
			$('select[name="f_ini_m"]').attr('disabled', true);
			$('select[name="f_ini_d"]').attr('disabled', true);
			$('select[name="f_fin_a"]').attr('disabled', true);
			$('select[name="f_fin_m"]').attr('disabled', true);
			$('select[name="f_fin_d"]').attr('disabled', true);
		}
	});
	...
	$('select[name="si_accionistas_table"]').change(function(event){
		(event.preventDefault) ? event.preventDefault() : event.returnValue = false;
		if($(this).val() == '-1'){//"input[name^=load_file]"
			$('table#accionistas_table select[name^="tipo_id"]').removeAttr('disabled');
			$('table#accionistas_table input[name^="identificacion"]').removeAttr('disabled');
			$('table#accionistas_table input[name^="nombre_accionista"]').removeAttr('disabled');
			$('table#accionistas_table input[name^="porcentaje"]').removeAttr('disabled');
			$('table#accionistas_table select[name^="publico_recursos"]').removeAttr('disabled');
			$('table#accionistas_table select[name^="publico_reconocimiento"]').removeAttr('disabled');
			$('table#accionistas_table select[name^="publico_expuesta"]').removeAttr('disabled');
			$('table#accionistas_table input[name^="declaracion_tributaria"]').removeAttr('disabled');
		}else if($(this).val() != ''){
			$('table#accionistas_table select[name^="tipo_id"]').val('').change().attr('disabled', true);
			$('table#accionistas_table input[name^="identificacion"]').val('').attr('disabled', true);
			$('table#accionistas_table input[name^="nombre_accionista"]').val('').attr('disabled', true);
			$('table#accionistas_table input[name^="porcentaje"]').val('').attr('disabled', true);
			$('table#accionistas_table select[name^="publico_recursos"]').val('').change().attr('disabled', true);
			$('table#accionistas_table select[name^="publico_reconocimiento"]').val('').change().attr('disabled', true);
			$('table#accionistas_table select[name^="publico_expuesta"]').val('').change().attr('disabled', true);
			$('table#accionistas_table input[name^="declaracion_tributaria"]').val('').attr('disabled', true);
		}
	});
