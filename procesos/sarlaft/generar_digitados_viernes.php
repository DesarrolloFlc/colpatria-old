...
function dataDigitadosColpatria(){
	...
	SELECT IF(cl.persontype = '1', 'NATURAL','JURIDICO') AS persontype,
	 ...
	 INNER JOIN param_tipo_norma AS tn ON(tn.id = da.tipo_norma_id)
	 INNER JOIN param_regimen AS pr ON(pr.id = da.regimen_id)
	 WHERE (fo.date_created BETWEEN '2007-01-01 00:00:00' AND '$hoy 23:59:59') 
	 /*WHERE (fo.date_created BETWEEN '2018-01-01 00:00:00' AND '2019-01-31 23:59:59') */
	   AND fo.status = 1
SQL;
		...
		if($conn->getNumeroRegistros() > 0){
            while($dat = $conn->sacarRegistro('str')){
				...
				$row = array(
					...
					trim(preg_replace("/\s+/", " ", $dat['fecharadicado'])), 
					...
					trim(preg_replace("/\s+/", " ", $dat['egresosmensualesemp'])), 
					trim(preg_replace("/\s+/", " ", $dat['patrimonio'])), 
					trim(preg_replace("/\s+/", " ", $dat['otrosingresosemp'])), 
					trim(preg_replace("/\s+/", " ", $dat['concepto_otrosingresosemp'])), 
