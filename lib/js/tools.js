...
//$.facebox.settings.opacity = 0.7
    ...
    $("form#reporteLotesPlanillas").submit(function(event) {
        ...
        $('#imgdownpdf').html(`<a href="generarReporteXLS.php?${datos}&consR=download" target="_blank"><img id="imgloading" src="../../images/icons/pdf_download_8.gif" />`);
        $('#imgdownpdf').qtip({
            content: {
...
//        return false;
    ...
    $("form#aprobarClientes").submit(function(event) {
        (event.preventDefault) ? event.preventDefault() : event.returnValue = false;
        if ($('input[name="fecha_envio"]', this).val() == '') {
            alert('Por favor seleccione la fecha de envio de Copatria');
            $('input[name="fecha_envio"]', this).focus();
            return false;
        }
        ...
        $.ajax({
            beforeSend: function() {
                $('input#buttonAprobar').attr('disabled', true);
                $('img#imgloading-aprobar').show();
            },
            data: $(form).serialize(),
            type: 'POST',
            url: '../includes/controllerRadicado.php',
            ...
            complete: function(jqXHR, textStatus){
                $('input#buttonAprobar').removeAttr('disabled');
                $('img#imgloading-aprobar').hide();
            },
            error: function(jqXHR, textStatus, errorThrown) {
            ...
            success: function(dato) {
                ...
                let strLote = `<div id="div_fechaenvio">
                    ...
                    <div style="display: flex; align-items: center;">
                        <input type="submit" id="buttonAprobar" class="button" value="Aprobar clientes del radicado">
                        <div style="width: 16px; height: 16px; padding-left: 5px;">
                            <img id="imgloading-aprobar" src="../../images/icons/loading.gif" style="display: none;" />
                        </div>
                    </div>
...
$.fn.getEstados = function(id) {
    const estados = ['Radicado', 'No Enviado', 'Recibido', 'Devuelto', 'Cancelado'];
    return estados[parseInt(id)] ?? estados[0];
};
$.fn.devolverItem = function(e, este, pos) {
    (e.preventDefault) ? e.preventDefault() : e.returnValue = false;
    const valorp = $(este).val().split('|');
    if (valorp[1] != '3') return false;
...
$.fn.devolverRadicadoForm = function(e, este) {
    (e.preventDefault) ? e.preventDefault() : e.returnValue = false;
    let seguir = true;
    $('input, select, textarea', este).each( function(index) {
    ...
    const pos = $('input[name="typepos"]', este).val();
    $.ajax({
        beforeSend: function() {
            $('img#imgloading-agregar', este).show();
            $('input#devolverItem').attr('disabled', true);
        },
        data: $(este).serialize(),
        type: 'POST',
        url: '../includes/controllerRadicado.php',
        ...
        success: function(dato) {
            ...
            if ($('input[name="opcion"]', este).val() == '1') {
                $(`input[name="cliente[${pos}]"]`).attr("disabled", "disabled");
                const newVal = $('input[name="clienteid_dev"]', este).val();
                $("tr#trcl_" + pos).append('<input type="hidden" name="cliente[' + pos + ']" value="' + newVal + '|3">');
            } else if ($('input[name="opcion"]', este).val() == '2') {
                $.fn.cambiarEstadoClientefn();
            }
        },
        complete: function () {
            $('img#imgloading-agregar', este).hide();
            $('input#devolverItem').removeAttr('disabled');
        },
        error: function(jqXHR, textStatus, errorThrown) {
            console.log(jqXHR, textStatus, errorThrown);
            alert(errorThrown+': '+jqXHR.responseText);
...
$.fn.cerrarVentana = function(check = false) {
...
$.fn.agregarCargaarchivos = function(e) {
    ...
    const strHTML = `<tr>
        <td width="8%"></td>
        <td>
            <input type="file" id="load_file" name="load_file[${cant}]">
            <div style="width:50px; display: inline; margin-left:20px;">
                <a href="#" onclick="$.fn.quitarCargaarchivos(event, this);">
                    <img src="../../resources/images/icons/cross_small.png" title="Eliminar archivo" alt="Eliminar" />
                </a>
            </div>
            <span class="input-notification attention png_bg">
                Utilice este icono para eliminar este cargue de la imagen en caso de haberlo agregado por error.
            </span>
        </td>
    </tr>`;
    $('table#files_loaders').append(strHTML);
};
$.fn.quitarCargaarchivos = function(e, este) {
    (e.preventDefault) ? e.preventDefault() : e.returnValue = false;
    const pos = $(este).parent().parent().find('#load_file').attr('name').split('[')[1].split(']')[0];
    const cant = $('input[name^="load_file"]').length;
    for (let i = pos; i < cant; i++) {
        if ((i + 1) <= cant) {
            const input = $(`input[name="load_file[${i}]"]`);
...
$.fn.radicadosyClientesxoficial = function(e) {//
    ...
    const datos = $("#radicadosyClientesxoficial").serialize();
    $.ajax({
        beforeSend: function() {
        },
        data: datos,
...
$.fn.NdocCliente = function(e, texto) {
    ...
    const datos = `idncliente=${texto}&estado=${$('#hddestado').val()}&idradicado=${$('#hddradicado').val()}&iditem=${$('#hdditem').val()}&docespecial=${$('#hddocespe').val()}&action=actualizaCliente`;
    $.ajax({
        data: datos,
        type: 'POST',
        url: '../includes/controllerRadicado.php',
...
$.fn.mostrarManual = function(e){
    (e.preventDefault) ? e.preventDefault() : e.returnValue = false;
    const strHTML = `<div id="object_pdf"></div>
    <script>
    const options = {
        width: "550px",
        height: "500px"
    };
    const file = "/Colpatria/procesos/radicado/UD-M-003_MANUAL_USUARIO_MODULO_RADICACION_COLPATRIA_V3.pdf";
...
$.fn.mostrarCambiarPassword = function(e){
    (e.preventDefault) ? e.preventDefault() : e.returnValue = false;
    $.facebox({
        div: '#change_password',
        checkClose: true
...
$.fn.cambiarPassword = function(e){
    (e.preventDefault) ? e.preventDefault() : e.returnValue = false;
    if ($('input[name="anterior_password"]', this).val().trim() === '') {
        $('div#cambiar-password-resultado-error div#mensaje-error').html("Debe digitar la contraseña actual del usuario.");
        $('div#cambiar-password-resultado-error').show(500);
        setTimeout(() => { $('div#cambiar-password-resultado-error').hide(300); }, 5000);
        return false;
    }
    if ($('input[name="nuevo_password"]', this).val().trim() === '') {
        $('div#cambiar-password-resultado-error div#mensaje-error').html("Debe digitar la nueva contraseña del usuario.");
        $('div#cambiar-password-resultado-error').show(500);
        setTimeout(() => { $('div#cambiar-password-resultado-error').hide(300); }, 5000);
        return false;
    }
    if ($('input[name="nuevo_password"]', this).val().trim() !== $('input[name="nuevo_password2"]', this).val().trim()) {
        $('div#cambiar-password-resultado-error div#mensaje-error').html("La nueva contraseña no coincide.");
        $('div#cambiar-password-resultado-error').show(500);
        setTimeout(() => { $('div#cambiar-password-resultado-error').hide(300); }, 5000);
        return false;
    }
    if ($('input[name="anterior_password"]', this).val().trim() === $('input[name="nuevo_password"]', this).val().trim()) {
        $('div#cambiar-password-resultado-error div#mensaje-error').html("La contraseña nueva debe ser diferente a la contraseña actual.");
        $('div#cambiar-password-resultado-error').show(500);
        setTimeout(() => { $('div#cambiar-password-resultado-error').hide(300); }, 5000);
        return false;
    }
    ...
    $.ajax({
        ...
        success: function(dato){
            if ((!dato.exito)) {
                $('div#cambiar-password-resultado-error div#mensaje-error').html(dato.error ? dato.error : 'Ocurrio un error al momento de realizar la consulta, contacte con el administrador.');
                $('div#cambiar-password-resultado-error').show(500);
                setTimeout(() => { $('div#cambiar-password-resultado-error').hide(300); }, 5000);
                if (!dato.error) console.log(dato);
                return false;
...
$.fn.crearUsuario = function(e) {
    (e.preventDefault) ? e.preventDefault() : e.returnValue = false;
    if ($('select[name="id_group"]', this).val() === "") {
        alert("Por favor valide el campo de perfil de usuario.");
        $('select[name="id_group"]', this).focus();
        return false;
    }
    ...
    if ($('input[name="identificacion"]', this).val() == "") {
        alert("Por favor valide el campo de identificación del usuario.");
        $('input[name="identificación"]', this).focus();
        return false;
    }
    const form = this;
    $.ajax({
        beforeSend: function(){
            $('input#crear-usuario').attr('disabled', true);
            $('img#imgloading-crear-usuario').show();
        },
        data: $(form).serialize(),
        type: 'POST',
        ...
        success: function(dato){
            ...
            $('#msg_adduser').html(dato.exito);
            $("#result_notif").css("display", "block");
            $('#divpadreoficial').slideUp('medium');
            $(form).reset();
        },
        complete: function(jqXHR, textStatus){
            $('input#crear-usuario').removeAttr('disabled');
            $('img#imgloading-crear-usuario').hide();
        },
        error: function(xhr, ajaxOptions, thrownError){
            console.log(xhr, ajaxOptions, thrownError);
            $('p.text-center > span').html(`Error(cargueBaseGestorVentas): ${xhr.status} Error: ${xhr.responseText}`);
