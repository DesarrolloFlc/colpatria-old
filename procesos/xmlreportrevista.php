...
$query = "SELECT cli.status_form,
				 ...
				 data.numerohijos AS numerohijos,
				 ...
				 data.totalactivos AS totalactivos,
				 data.totalpasivos AS totalpasivos,
				 cli.persontype	
			FROM client cli 
		   ...
		   WHERE cli.persontype = '1'
		     AND formu.date_created BETWEEN '2014-10-24 00:00:00' AND '2014-10-30 23:59:59'
		   GROUP BY data.documento
		   ORDER BY formu.date_created";
//AND formu.date_created <= '2014-02-17 23:59:59'
//84547LIMIT 0 , 42273
//LIMIT 42274 , 84547
//WHERE cli.persontype = '1'
...
$outDir = '../xmlreport/outrevista';
@mkdir($outDir);
$file = fopen( $outDir.'/super_nat_'.date('Ymdhi').'.xml', 'w');
if (!$file) {
...
while($row = mysqli_fetch_assoc($result)){
	...
	if($row['numerohijos'] == 'SD')$row['numerohijos'] = 0;
	...
	<Persona>
		...
		<EstadoCivilPersona>
			...
			<NumeroHijos>{$row['numerohijos']}</NumeroHijos>
		</EstadoCivilPersona>
		$nivelestrato
		...
		<TotalActivos>{$row['totalactivos']}</TotalActivos>
		<TotalPasivos>{$row['totalpasivos']}</TotalPasivos>
		<EstadoPersona>
			<IDEstado>1</IDEstado>
...
function IDunidadMedida($unidad){
	return $unidad == 'pesos' ? 1 : ($unidad == 'SMMV' ? 2 : 0);
}
function getIdGenero($genero){
	$gen = substr($genero, 0, 1);
	return $gen === "M" ? 2 : ($gen === "F" ? 1 : 3);
}
