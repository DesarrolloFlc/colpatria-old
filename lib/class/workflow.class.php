...
//Workflow_migracion
    ...
    static function cambiarEstado($documento, $persontype){
        $SQL = "UPDATE workflow SET estado = 1 WHERE documento = '$documento' AND persontype = $persontype";
        if(mysqli_query($GLOBALS['link'], $SQL))
            return true;
        else
            return false;
    }
    static function verificarCliente($documento, $persontype){
        $SQL = "SELECT * FROM workflow WHERE documento = '$documento' AND persontype = $persontype";
