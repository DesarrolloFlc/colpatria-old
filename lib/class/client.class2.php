...
//require_once ($_SERVER['DOCUMENT_ROOT'].'/Aplicativos.Serverfin04'.'/Colpatria/includes.php');
...
class Client{
    ...
    function getEstadoInformacion($id_client) {
        $conn = new Conexion();
        $query = <<< SQL
...
SQL;
        if($conn->consultar($query)){
            if($conn->getNumeroRegistros()){
                $dat = $conn->sacarRegistro('str');
                if(isset($dat['valid'])){
                    return "Vigente hasta " . date("Y-m-d", strtotime($dat['date']));
                }else{
