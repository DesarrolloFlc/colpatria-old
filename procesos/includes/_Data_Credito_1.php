...
fputcsv($fh, array('DOCUMENTO', 'EXITE?', 'SGV', 'CAPI', 'Ultimo movimiento', 'Vigente Hasta'), ';');
$cexisten = 0;
$csgv = 0;
$ccapi = 0;
$cnoexisten = 0;
...
for ($i = 1; $i < $n; $i++) {
    ...
    if ($id["id"] != "" && $id["id"] != NULL) {
        ...
        $info = $u->getinfoclientesactualizados($id["id"]);
//        echo ''.$info;
        fputcsv($fh, array($documento, 'SI', $id["type"], $id["capi"], $info["ultimo_m"], $info["Valido_"]), ';');
    } else {
        $cnoexisten++;
