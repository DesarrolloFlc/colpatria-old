...
foreach($datos as $dato){
	...
	if($dato['persontype'] == 1){//PERSONA NATURAL
		...
		$lugarnacimiento = '';
		//
		if(is_null($dato['nacionalidad_otra']))
			$nacionalidad_otra = 'SD';
		elseif($dato['nacionalidad_otra'] == '1')
			$nacionalidad_otra = 'SI';
		elseif($dato['nacionalidad_otra'] == '2')
			$nacionalidad_otra = 'NO';
		else
			$nacionalidad_otra = 'SD';
		//
		if(is_null($dato['productos_exterior']))
			$productos_exterior = 'SD';
		elseif($dato['productos_exterior'] == '1')
			$productos_exterior = 'SI';
		elseif($dato['productos_exterior'] == '2')
			$productos_exterior = 'NO';
		else
			$productos_exterior = 'SD';
		//
		if(is_null($dato['cuentas_monedaextranjera']))
			$cuentas_monedaextranjera = 'SD';
		elseif($dato['cuentas_monedaextranjera'] == '1')
			$cuentas_monedaextranjera = 'SI';
		elseif($dato['cuentas_monedaextranjera'] == '2')
			$cuentas_monedaextranjera = 'NO';
		else
			$cuentas_monedaextranjera = 'SD';
		//
		if(is_null($dato['obligaciones_otropais']))
			$obligaciones_otropais = 'SD';
		elseif($dato['obligaciones_otropais'] == '1')
			$obligaciones_otropais = 'SI';
		elseif($dato['obligaciones_otropais'] == '2')
		...
		elseif($dato['formato_paisn'] == '8')
			$formato_paisn = 'Formato W8';
		elseif($dato['formato_paisn'] == '9')
		...
		elseif($dato['formato_nacio'] == '8')
			$formato_nacio = 'Formato W8';
		elseif($dato['formato_nacio'] == '9')
		...
		elseif($dato['formato_oblip'] == '8')
			$formato_oblip = 'Formato W8';
		elseif($dato['formato_oblip'] == '9')
		...
		$i_a = array(
			...
			$dato['resultadoentrevista'], $dato['nombreintermediario'], $dato['usuario'], $ultimoestado, $dato['tipoformulario'],
	...
	}elseif($dato['persontype'] == 2){//PERSONA JURIDICA
		//NOMBRE COMPLETO
		$nom_com = trim($dato['nombres'].' '.trim($dato['primerapellido']).' '.trim($dato['segundoapellido']));
		//FECHA RADICADION COLPATRIA
		$fecha_rad_col = $dato['fecha_rad_col'];
		...
		//
		$cliente_fatca = 'No';
		if((!is_null($dato['obligaciones_pais']) && $dato['obligaciones_pais'] != 'CO' && $dato['obligaciones_pais'] != 'SD'))
			$cliente_fatca = 'Si';
		$i_a = array(
			'JURIDICO', $dato['sucursal'], $dato['area'], $dato['id_official'], $dato['log_planilla'], $dato['log_lote'], $dato['marca'], 
			$dato['tipo_cliente'], 'NIT', $dato['razonsocial'], $dato['document'], $dato['document'], $dato['digitochequeo'], 
			$fecha_rad_col, $fecha_envio, $fecha_recibido, $dato['fechasolicitud'], $date_created, $act_eco_prin, 
			$dato['detalleactividadeconomicappal'], $dato['tipo_emp'], 'NA', 'NA', 'NA', 'NA', 'NA', 'NA', 'NA', 'NA', 'NA', 'NA', 'NA', 
			'NA', 'NA', 'NA', 'NA', 'NA', $dato['ciudadoficina'], $dato['direccionoficinappal'], $dato['nomenclatura'], 
			(empty($dato['telefonoresidencia']) ? '0' : $dato['telefonoresidencia']), (empty($dato['telefonoficina']) ? '0' : $dato['telefonoficina']), 
			(empty($dato['celular']) ? '0' : $dato['celular']), (empty($dato['celularoficina']) ? '0' : $dato['celularoficina']), 
			(empty($dato['faxoficina']) ? '0' : $dato['faxoficina']), (empty($dato['correoelectronico']) ? 'SD' : $dato['correoelectronico']), 'NA', 
			(empty($dato['ciiu']) ? 'SD' : $dato['ciiu']), $dato['ciudadoficina'], $dato['tipoempresaemp'], 'NA', $dato['ingresosmensualesemp'], 
			$dato['egresosmensualesemp'], $dato['otrosingresosemp'], $dato['concepto_otrosingresosemp'], $dato['activosemp'], $dato['pasivosemp'], 
			$dato['monedaextranjera'], $dato['tipotransacciones'], $nom_com, $dato['tipo_identificacion'], $dato['documento'], $dato['huella'], 
			$dato['firma'], $dato['entrevista'], $dato['lugarentrevista'], $dato['fechaentrevista'], $dato['hora_entrevista'], 
			$dato['resultadoentrevista'], $dato['nombreintermediario'], $dato['usuario'], $ultimoestado, $dato['tipoformulario'], 'NA', 'NA', 'NA', 
			$productos_exterior, $cuentas_monedaextranjera, 'NA', 'NA', 'NA', 'NA', $formato_oblip, $cliente_fatca,
			(isset($dato['productos'][0]['tipo']) ? $dato['productos'][0]['tipo'] : ''), (isset($dato['productos'][0]['identificacion_producto']) ? $dato['productos'][0]['identificacion_producto'] : ''), 
			(isset($dato['productos'][0]['entidad']) ? $dato['productos'][0]['entidad'] : ''), (isset($dato['productos'][0]['monto']) ? $dato['productos'][0]['monto'] : ''),
			(isset($dato['productos'][0]['pais']) ? $dato['productos'][0]['pais'] : ''), (isset($dato['productos'][0]['ciudad']) ? $dato['productos'][0]['ciudad'] : ''), 
			(isset($dato['productos'][0]['moneda']) ? $dato['productos'][0]['moneda'] : ''),
			...
			(isset($dato['productos'][1]['pais']) ? $dato['productos'][1]['pais'] : ''), (isset($dato['productos'][1]['ciudad']) ? $dato['productos'][1]['ciudad'] : ''), 
			(isset($dato['productos'][1]['moneda']) ? $dato['productos'][1]['moneda'] : ''),
			(isset($dato['socios'][0]['tipo']) ? $dato['socios'][0]['tipo'] : ''), (isset($dato['socios'][0]['identificacion']) ? $dato['socios'][0]['identificacion'] : ''),
			...
			(isset($dato['socios'][1]['tipo']) ? $dato['socios'][1]['tipo'] : ''), (isset($dato['socios'][1]['identificacion']) ? $dato['socios'][1]['identificacion'] : ''),
			...
			(isset($dato['socios'][2]['tipo']) ? $dato['socios'][2]['tipo'] : ''), (isset($dato['socios'][2]['identificacion']) ? $dato['socios'][2]['identificacion'] : ''),
			...
			(isset($dato['socios'][3]['tipo']) ? $dato['socios'][3]['tipo'] : ''), (isset($dato['socios'][3]['identificacion']) ? $dato['socios'][3]['identificacion'] : ''),
			...
			(isset($dato['socios'][4]['tipo']) ? $dato['socios'][4]['tipo'] : ''), (isset($dato['socios'][4]['identificacion']) ? $dato['socios'][4]['identificacion'] : ''),
...
function generarDataReporte($fecha_ini, $fecha_fin, $comp)
{
	...
	$SQL = "SELECT t2.persontype, t4.sucursal, t5.description AS area, t2.id_official, t2.log_planilla, t2.log_lote, t2.marca, 
			...
			t2.resultadoentrevista, t2.nombreintermediario, t32.name AS usuario, t6.descripcion AS tipoformulario, t2.client_id, t2.celularoficina, 
...
{
	$SQL = "SELECT t1.id, t1.data_id, t1.tipo, t1.identificacion_producto, t1.entidad, t1.monto, 
			t1.pais AS pais_id, t2.description AS pais, 
			t1.ciudad AS ciudad_id, CONCAT(t3.ciudad, ', ', t3.departamento_nombre) AS ciudad,
			t1.moneda AS moneda_id, CONCAT(t4.pais, ', ', t4.moneda) AS moneda
...
{
	$SQL = "SELECT t1.id, t1.data_id, t1.tipo_id, t2.description AS tipo,
			t1.identificacion, t1.nombre_accionista, t1.publico_recursos, 
			t1.publico_grado, t1.publico_reconocimiento, t1.declaracion_tributaria, 
			t1.declaracion_pais, t3.description AS declaracionpais
