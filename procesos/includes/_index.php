...
function clientsIlocalizadosNaturales()
{
    ...
    if ($conn->getNumeroRegistros() > 0) {
        while ($consulta = $conn->sacarRegistro()) {
            ...
            $conn1->consultar($SQL1);
            if ($conn1->getNumeroRegistros() == 0) {
                $objetos[] = $consulta;
            }
        }
    }
    ...
    for ($i = 1; $i < $n; $i++) {
        ...
        if ($tipodocumento != 'NIT' || $tipodocumento != 'N') {
            ...
            if ($conn->getNumeroRegistros() > 0) {
                ...
                echo $SQL2 . "<br>";
                $conn2->consultar($SQL2);
                if ($conn2->getNumeroRegistros() == 0) {
...
function UpdateANDGetClientsCapiJuridico()
{
    ...
    for ($i = 1; $i < $n; $i++) {
        ...
        $fechaexpedicionP = explode('/', trim($datos_leer[0]));
...
function insertGestionNoContacto()
{
    ...
    for ($i = 1; $i < $n; $i++) {
        ...
        if ($conn->getNumeroRegistros() > 0) {
            $consulta = $conn->sacarRegistro();
            echo "SE_ENCONTRO CLIENTE CON ID {$consulta['id_client']} y CEDULA $ced<br><br>";
            ...
            echo "NO SE ENCONTRO CLIENTE CON CEDULA $ced<br><br>";
            ...
            $conn1->consultar($SQLS);
            if ($conn1->getNumeroRegistros() > 0) {
                $consulta = $conn1->sacarRegistro();
                $id_cliente = $consulta['id_client'];
...
function insertGestionNoContactoSeguros()
{
    ...
    for ($i = 1; $i < $n; $i++) {
        ...
        if ($conn->getNumeroRegistros() > 0) {
            $consulta = $conn->sacarRegistro();
            echo "SE_ENCONTRO CLIENTE CON ID {$consulta['id_client']} y CEDULA $ced<br>";
            $id_cliente = $consulta['id_client'];
            $id_form = $consulta['id_form_'];
            $persontype = $consulta['typeperson'];
            $documento = $consulta['documento'];
            ...
            echo "NO SE ENCONTRO CLIENTE CON CEDULA $ced<br>";
            /* $conn1 = new Conexion();
              $SQL1 = "SELECT * FROM data WHERE documento = '$ced'";
              $conn1->consultar($SQL1);
              $numcot = $conn1->getNumeroRegistros();
              if ($numcot == 0){
        ...
        if ($conn->getNumeroRegistros() > 0) {
            ...
            if ($cantidad == 1) {
                ...
                /* $conn2 = new Conexion();
                  ...
                  LIMIT 1";
                  echo $SQL1."<br>";
                  $conn2->consultar($SQL1);
                  if($conn2->getNumeroRegistros() > 0){ */
                /* $consulta2 = $conn2->sacarRegistro();
    ...
    if ($conn->getNumeroRegistros() > 0) {
        ...
        while ($consulta = $conn->sacarRegistro()) {
            ...
            $SQLF = "SELECT id
					 FROM client
					 WHERE document = '$document'";
            $conn1->consultar($SQLF);
            if ($conn1->getNumeroRegistros() > 0) {
                $consul1 = $conn1->sacarRegistro();
              ...
              echo "documento $document mas de una vez en oficiales<br><br>";
                ...
                if ($conn->getNumeroRegistros() == 0) {
                    ...
                    if ($conn->ejecutar($SQLI)) {
                        ...
                        echo $SQLI1 . "<br><br>";
                        if ($conn->ejecutar($SQLI1)) {
                            $uus = 'USER-IG';
                            fwrite($fp, "$ultimaId;$fecha_creacion;$namecomple;$document;DOC FINDER;$permisos;$correo;$father;$correopadre;$usuario;$uus;OFICIAL-IG" . PHP_EOL);
                        } else {
                            $uus = 'USER-IE';
                            fwrite($fp, "$ultimaId;$fecha_creacion;$namecomple;$document;DOC FINDER;$permisos;$correo;$father;$correopadre;$usuario;$uus;OFICIAL-IE" . PHP_EOL);
                        }
                    } else {
                        fwrite($fp, "NULL;$fecha_creacion;$namecomple;$document;DOC FINDER;$permisos;$correo;$father;$correopadre;$usuario;USER-IE;OFICIAL-IE" . PHP_EOL);
                    }
                } else {
...
function updateWorkflowPersontype()
{
    ...
    if ($conn->getNumeroRegistros() > 0) {
        while ($consulta = $conn->sacarRegistro()) {
            ...
            echo $SQL1 . "<br>";
            $conn1->consultar($SQL1);
            $cant1 = $conn1->getNumeroRegistros();
            if ($cant1 == 1) {
...
function repararRadicacionFecha_RadicacionFisicos()
{
    ...
    $SQL = "SELECT id FROM `radicados` 
			...
			AND estado = 2";
    $conn->consultar($SQL);
    echo "SE ENCONTRARON: " . $conn->getNumeroRegistros() . "<br><br><br>";
    while ($consulta = $conn->sacarRegistro()) {
        $lote = $consulta[0];
    ...
    for ($i = 1; $i < $n; $i++) {
        ...
        if ($id_cl_clientes) {
            if (strlen($datos_leer) === 194) {
                ...
                $id_clientes_sub = '';
                if ($conn->ejecutar($SQLS))
                    $id_clientes_sub = $conn->ultimaId();
                else
            ...
            if (strlen($datos_leer) === 72) {
                ...
                if ($conn->ejecutar($SQLP))
                    $id_presunta = $conn->ultimaId();
                else
                    $id_presunta = false;
            }
        }
...
{
    $link = mysqli_connect('localhost', 'colpatria_sgd', 'colpatria_sgd') or die('No se pudo conectar: ' . mysqli_error($link));
    mysqli_select_db($link, 'paises_mundo') or die('No se pudo seleccionar la base de datos');
    ...
    while ($consulta = mysqli_fetch_array($resp, MYSQLI_NUM))
...
function imagenesExistente3()
{ //2017-06-22 Clientes no vigentes solo si no tienen imagenes, no importa la fecha
    $conn = new Conexion();
    $ruta = '/var/www/html/Aplicativos.Serverfin04/images_colpatria/';
    /*header("Cache-Control: must-revalidate, post-check=0, pre-check=0");
    header("Content-Description: File Transfer");
