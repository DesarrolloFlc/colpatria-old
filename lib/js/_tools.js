...
//$.facebox.settings.opacity = 0.7
    ...
    $("#verificarCambioEstadoRadicado").submit(function(event) {
        (event.preventDefault) ? event.preventDefault() : event.returnValue = false;
        if ($('form#verificarCambioEstadoRadicado input#id').val() == '') {
            $('#boxError').html('Por favor digite el numero de radicado.');
            $.facebox({div: '#boxError'});
            $('form#verificarCambioEstadoRadicado input#id').focus();
            return false;
        }
        if ($('form#verificarCambioEstadoRadicado input#documento').val() == '') {
            $('#boxError').html('Por favor digite el numero de documento del cliente a cambiar el estado.');
            $.facebox({div: '#boxError'});
            $('form#verificarCambioEstadoRadicado input#documento').focus();
            return false;
        }
        var conf = true;
        if (conf) {
            var datos = $("#verificarCambioEstadoRadicado").serialize();
            //alert(datos);
            $.ajax({
                beforeSend: function() {
                },
                ...
                success: function(dato) {
                    ...
                    if(dato.exito){
                        if(dato.item){
                            var estadoa = '';
                            var estados = ["", "No fue enviado", "Recibido", "Devuelto"];
                            var excluido = 0;
                            if(dato.item.estado = 1){
                ...
                success: function(dato) {
                    ...
                    if (dato.exito) {
                        ...
                        var strHTML = '<td>' + dato.radicado['id'] + '</td>' +
                                ...
                                '<td><a href="generarReportePDF.php?idradicado=' + dato.radicado['id'] + '&downpdf=download"><img src="../../resources/images/icons/pdf_icon.gif" title="Descargar PDF" alt="Descargar PDF" /></a></td>';
                        strItems = '<tr><td></td><td width="20%"></td><td width="68%"></td><td width="2%">todo</td><td width="2%">nada</td><td width="10%">todo</td></tr>';
                        strItems += '<tr><td></td><td width="20%"></td><td width="68%"></td><td width="2%">'+
                                    '<input type="radio" name="cliente_select" id="cliente_select" value="" onchange="$.fn.seleccionarTodo(event,this,1);">'+
                                    '</td><td width="2%"><input type="radio" name="cliente_select" id="cliente_select" value="" onchange="$.fn.seleccionarTodo(event,this,3);"></td><td width="10%">'+
                                    '<input type="radio" name="cliente_select" id="cliente_select" value="" onchange="$.fn.seleccionarTodo(event,this,2);"></td></tr>';
                        strItems += '<tr>' +
                                '<td></td>' + //SKRV Editar documento
                                '<td width="20%">Documento</td>' +
                        ...
                        if (dato.items) {
                            ...
                            for (var i = 0; i < lengtitems; i++) {
                                ...
                                strItems += '<td><a href="#" title="Edit" onclick="$.fn.edCliente( \'' + dato.radicado['estado'] + '\', \'' + dato.radicado['id'] + '\', \'' + dato.items[i].id + '\', \'' + dato.items[i].documento_especial + '\',event );"><img src="../../resources/images/icons/pencil.png" alt="Edit" /></a></td>'; //SKRV Editar documento
    ...
    function validaAgregarCliente(docespe) {
        if (confirm('Esta seguro que la informaci\xF3n que radic\xF3 y va a enviar esta completa, debidamente diligenciada y tiene todos los documentos adicionales para cumplir con los p\xE1rametros exigidos por SARLAFT, y que son su responsabilidad, y por lo tanto este cliente no tendr\xE1 ninguna causal de devoluci\xF3n?')) {
            ...
            if ($('#chkoptesp').is(':checked'))
                optesp = "Si";
            var str = '<tr class="tr_content"><td>'+nom.toUpperCase()+'</td><td>'+ced+'</td><td>'+optesp+'</td>'+
                    '<td align="center" valign="middle" class="td_formatopiso"><a href="#'+nom+'|'+ced+'|'+docespe+'" onclick="$(this).hidetr(event);"><img src="../../resources/images/icons/cross_circle.png" title="Eliminar" alt="Eliminar" /></a></td></tr>';
...
//SKRV valida si el usuario tiene formulario
        if ($('#chkoptesp').is(':checked')) {
            $.ajax({
...
// FIN SKRV valida si el usuario tiene formulario
